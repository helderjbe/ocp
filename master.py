...
# Relative location of the AUV. Init with None
# Height, angle
loc = [None, None]
...
def get_data(header, file=None):
    ...
    while True:
        ...
        if header==None:
            print("In function get_data(): Error receiving what was requested")
            logging.critical("In function get_data(): Error receiving what was requested")
            return None
        elif header=='Error':
            ser.flushInput()
            ser.write(nak_pack)
    ...
    while True:
        ...
        try:
            ...
            if seq <= 7:
                terminator(1) # Respond to communication attempts by sending out an ACK
                if file is None:
                    return b"".join(buffer)
                else:
                    print ("File received successfully")
                    logging.debug("In function get_data: File received successfully")
                    return 1
...
def send(stream, length):
    ...
    print("In function send(): Unexpected error occured")
    logging.critical("In function send(): Unexpected error occured")
    return None

def location():
        global loc
        
        # Get the current AUV location and store in global vars
        location=get_data(send("gl", 2))
        if location!=0 and location!=1 and location!=None:
            lst=location.split()
        else:
            return 0
        ...
        if loc == [None, None]:
            logging.debug("> Location set: Height: N/A Angle: N/A")
            print("Height: N/A Angle: N/A")
        else:
            logging.debug("> Location set: Height: %s, Angle: %s" %(loc[0], loc[1]))
            print("Height: %s, Angle: %s" %(loc[0], loc[1]))
            
        return 1

# CMD shell - Gets user input command and parses it infinitely
# Options implemented:
# send [msg] - sends a command message (example of motor control)
# location - request AUV location (example of error map control and other uses)
# getfile [file] - request file from AUV (example of longer use of communication - can request logs)
class OCP(cmd.Cmd):
    intro = 'Welcome to the optical communication protocol shell.   Type help or ? to list commands.\n'
    prompt = '> '
    
    def do_send(self, *arg):
        'send [msg]'
        logging.debug('> send %s' % arg)
        
        if location():
            send("sm " + arg[0], 3+len(arg[0]))
        else:
            print("Failed to retrieve AUV location")
            return
    ...
    def do_getfile(self, arg):
        'getfile [file]'
        logging.debug('> getfile %s' % arg)
        if location():
            #sim
            start= time.time()
            get_data(send("fg " + arg, 3+len(arg)), arg)
