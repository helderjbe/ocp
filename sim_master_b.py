...
# Efficiency
seff = [(((9.0*dec[d])/nsym[d])*100) for d in range(len(nsym)-1)] + [0]
npow = [((float(nsym[npowk])/n)-9.0) for npowk in range(len(nsym)-1)] + [0]
...
plt.grid(True)
print ('Saving...')
plt.savefig('sim-master-b-decode.jpg', bbox_inches='tight')
...
plt.grid(True)
print ('Saving...')
plt.savefig('sim-master-b-naks.jpg', bbox_inches='tight')
...
plt.grid(True)
print ('Saving...')
plt.savefig('sim-master-b-efficiency.jpg', bbox_inches='tight')